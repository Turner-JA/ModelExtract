...
Extract_BRMSOLD <- function(brms_model, fontsize, filename){
  ...
  ## Headers (Model & Formula rows left empty for compose())
  header0FE <- data.frame(Parameter = "", Estimate = "", CI_low = "", CI_high="", pd="", Meaningful="")
  header0.5FE <- data.frame(Parameter = "", Estimate = "", CI_low = "", CI_high="", pd="", Meaningful="")
  header1FE <- data.frame(Parameter = "Fixed Effects", Estimate = "", CI_low = "", CI_high="", pd="", Meaningful="")
  header2FE <- data.frame(Parameter = "Predictors", Estimate = "Log Odds",
                          CI_low = "CI low", CI_high="CI high", pd="pd", Meaningful="Meaningful")
  FEs <- rbind(header0FE, header0.5FE, header1FE, header2FE, FEs)
